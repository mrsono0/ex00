...
public class CMController2 {

	public static void main(String[] args) {
		// 고객 정보를 저장할 자료구조 선언
		ArrayList<Customer> custList = new ArrayList<>();
		CMDAO2 cmdao = new CMDAO2();
		...
		while (true) {
			...
			case 'i':
				...
				cmdao.insertCustomerData(custList, scan);
			...
			case 'p':
				...
				if (index <= 0) {
					...
					index--;
					cmdao.printCustomerData(custList, index, scan);
				}
				break;
			case 'n':
			...
			case 'u':
				...
				if ((index >= 0) && (index < count)) {
					...
					cmdao.updateCustomerData(custList, index, scan);
				} else {
			...
			case 'd':
				...
				if ((index >= 0) && (index < count)) {
					...
					cmdao.deleteCustomerData(custList, index);
				} else {
